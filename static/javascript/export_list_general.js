    ...
    $.ajax({
        ...
        if (res["result"] == 0) {
            ...
            for (var i = 0; i < res.length; i++) {
                ...
                $(".editRow", template).attr("onclick", "javascript: location.href='/compare_version/"+ res[i].uuid +"'");
                $(".delRow", template).data("uuid", res[i].uuid);

                $('#rowData').append(template);
            }

            myTable = $('#tableListExportGeneral').DataTable({
                "info": false,
                // "bServerSide": true,
            ...
            $("#tableListExportGeneral_filter label").before('<label><select id="selectFileType" aria-controls="list" class="form-control form-control-sm"><option value="">篩選檔案類型</option></select></label>');
            $("#selectFileType").append(fileType.join("\n"));
            $('#selectFileType').on('change', function () {
                myTable.columns(1).search(this.value).draw();
            });
            $("#tableListExportGeneral_filter input").on('keyup', function () {
